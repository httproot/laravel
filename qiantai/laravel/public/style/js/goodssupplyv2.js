...
function search(isSearchInResults, isPaging) {
    ...
    parseurl = url + "?" + searchParameters.substr(1, searchParameters.length - 1);
   ...
   async: false,
   url: "ajax",
   data:"parseurl=" + parseurl,
   headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
   success: function(data){
    
    console.log(data);
	 //var dtt=JSON.stringify(data);
   //var dtt=eval((JSON.stringify(data))[0]);
  // $('.tbody .t-1 span').html(dtt.pid);
   }
},'json')
